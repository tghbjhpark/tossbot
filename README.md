...
- **손절 기능 (`stop_loss_count`)**: `config/ticker.json`에 손절 수량(`stop_loss_count`)을 지정하면, **정규장 시간(한국 09:00~15:20 / 미국 09:30~16:00 EST)**에 매도 예정가가 가장 높은 **최상단 포지션부터 카운트 수만큼 즉시 손절(매도)을 진행**합니다. 손절 처리 후 DB 업데이트를 완료하고 해당 설정값은 자동으로 `0`으로 리셋됩니다. 장 외 시간에는 정규장 개장 시까지 대기합니다.
