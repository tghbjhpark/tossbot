...
* **일일 밴드 감시 및 리밸런싱**: 매일 지정된 미국 뉴욕 시각(기본 11:00 AM)에 주식 평가액($E$)을 측정하여 밴드 범위($V_{min} \sim V_{max}$, 기본 $\pm 15\%$)를 벗어날 때만 리밸런싱 주문을 실행합니다.
...
* **`min_trade_amount`** (Float): 최소 거래 허들 금액 (기본값 `$10.0`).
* **`rebalance_hour_us`** (Integer): 하루 1회 감시할 뉴욕 시각 (기본값 `11` = 11:00 AM EST).
