...
- **손절 기능 (`stop_loss_count`)**: `config/ticker.json`에 손절 수량(`stop_loss_count`)을 지정하면, 봇 실행 시 매도 예정가가 가장 높은 **최상단 포지션부터 카운트 수만큼 즉시 손절(매도)을 진행**합니다. 손절 처리 후 DB 업데이트를 완료하고 해당 설정값은 자동으로 `0`으로 리셋됩니다.
...
* **`stop_loss_count`** (Integer, GRID 전용, Optional): 손절할 포지션 개수. 매도 예정가가 가장 높은 최상단 포지션부터 지정된 수만큼 즉시 손절(매도)을 실행하고 DB를 업데이트하며, 완료 시 자동으로 `0`으로 리셋됩니다.
